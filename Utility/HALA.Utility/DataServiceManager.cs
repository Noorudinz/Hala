...
namespace HALA.Utility
{
    public class DataServiceManager
    {
        ...
        {
            using (var proxy = new HALAWebClient())
            {
                try
                {
        ...
        public T1 PostData<T1, T2>(string method, T2 t2) where T1 : class
        {
            using (var proxy = new HALAWebClient())
            {
                try
                {
                    ...
                    if (IsLoginAction)
                    {
                        var tokenSecurityInfo = t2 as HALA.DTO.RequestResponseWrappers.OAuthRequest;
        ...
        {
            var response = new HALA.DTO.RequestResponseWrappers.ApiResponse<string>();
        ...
        /// </summary>
        private class HALAWebClient : HttpClient
        {
        }
